...
def link_tr_with_bug(tc_fail,have_tr):

    print("-----------------------------------------Function link_tr_with_bug called-----------------------------------")
    #bug_ids = []
    
    for i in range(len(tc_fail.keys())):
        tr_id = have_tr[i]
        bug_id = tc_fail[tc_fail.keys()[i]]
...
def create_tr_multiple(not_have_tr, link_type, owner, test_cycle_name, tc_id_to_title, release, reason_other, id_list):
    ...
    for chunk_num in range(num_chunks):
        ...
        for i in range(len(chunk_tc_ids)):
            ...
            if tc_id_to_title[chunk_tc_ids[i]][2].lower() in ["passed", "pass"]:
                status_value = "complete"
                reason_value = "pass"
            elif tc_id_to_title[chunk_tc_ids[i]][2].lower() in ["failed", "fail"]:
                status_value = "complete"
                reason_value = "fail"
            else:
                status_value = "blocked"
                reason_value = tc_id_to_title[chunk_tc_ids[i]][2]
...
def update_tr_multiple(have_tr, tc_id_to_title, owner, reason_other):
    ...
    for chunk_num in range(num_chunks):
        ...
        for i in range(len(chunk_tc_ids)):
            test_result_id = chunk_tc_ids[i]
            if tc_id_to_title[test_result_id][2].lower() in ["passed", "pass"]:
                status_value = "complete"
                reason_value = "pass"
            elif tc_id_to_title[test_result_id][2].lower() in ["failed", "fail"]:
                status_value = "complete"
                reason_value = "fail"
            else:
                status_value = "blocked"
                reason_value = tc_id_to_title[test_result_id][2]
            chunk_payload.append({
                "rowNum": i,
                "id": int(test_result_id),
                "fieldValues": [
...
def read_tc_ids(csv_file_path):
    ...
    with open(csv_file_path, encoding="utf-8") as csv_file:
        ...
        for row in csv_reader:
            ...
            if tc_result in ["fail", "failed", "Fail", "Failed", "FAIL", "FAILED"]:
                tc_fail[tc_id]= row[10]
            
            tc_id_to_title[tc_id] = [tc_title,tc_config, tc_result]
    ...
    print("---------------------------TC_IDS---------------------------------------------------")
    print(tc_ids)
    return tc_ids,tc_id_to_title,tc_fail
...
if __name__ == "__main__":
    ...
    not_have_tr = [] #list conatining tc_id which don't have valid tr
    id_list = []
    tc_id_to_title={} #dictionary to maintain details required of each tc from csv
    tc_fail ={}
    ...
    tc_ids, tc_id_to_title ,tc_fail = read_tc_ids(path)
    ...
    if len(have_tr) > 0:
        ...
        update_tr_multiple(have_tr, tc_id_to_title, owner, reason_other)
    if len(not_have_tr) > 0:
        print("---------------------------calling create_tr_multiple---------------------------------------------------")
        id_list = create_tr_multiple(not_have_tr, link_type, owner, test_cycle_name, tc_id_to_title, release, reason_other, id_list)
    if len(id_list) > 0: 
        print("----------------------------calling update new created tr--------------------------------------------------")
        update_new_tr(id_list)
    
    if len(tc_fail.keys()) > 0:
        have_tr,not_have_tr = fetch_tc_id_tr_id(tc_fail.keys(),test_cycle_name,tc_id_to_title)
        link_tr_with_bug(tc_fail, have_tr)
