#code to bulk update the test results and link bug-id in case of fail test cases in PROD server
...
def link_tr_with_bug(have_tr, csv_file_path):
    ...
    for i in range(len(have_tr)):
        tr_id = have_tr[i]
        bug_id = bug_ids[i]    
        url = f'https://hsdes-api.intel.com/rest/article/{tr_id}/links'  # use an f-string to include the tr_id value in the URL
