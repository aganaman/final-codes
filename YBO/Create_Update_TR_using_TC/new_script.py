...
def create_tr_multiple(not_have_tr, status_value, reason_value, link_type, owner, test_cycle_name, config, release):
    ...
    for chunk_num in range(num_chunks):
        ...
        for i in range(len(chunk_tc_ids)):
            ...
            title = tc_id_to_title[chunk_tc_ids[i]] + " [TC ID" + chunk_tc_ids[i] + "]"
            release_aff = release
            chunk_payload.append({
                ...
                "fieldValues": [
                    ...
                    {
                        "test_result.configuration": config
                    }
                ]
            })
        print(chunk_payload)
...
def update_tr_multiple(have_tr, test_cycle_name, config, status_value, reason_value, owner):
    # Construct the payload for the PUT request
    payload = []
    for i in range(len(have_tr)):
        test_result_id = have_tr[i]
        payload.append({
            "rowNum": i,
...
def fetch_tc_id_tr_id(tc_ids, test_cycle_name, config):
    have_tr=[]
    not_have_tr=[]
    print("------------------------------------------in fetch tc tr id-------------------------------------------------------------------")
    print(tc_ids)
    eql_test_case_definitions = "\"eql\":\"select id, title, release, submitted_date where " \
                        "tenant='central_firmware' and subject='test_result' and test_result.test_cycle contains '{0}'  and parent_id IN ({2})" \
                        "\"".format(test_cycle_name,config,",".join(tc_ids))
...
if __name__ == "__main__":
    ...
    have_tr, not_have_tr = fetch_tc_id_tr_id(tc_ids,test_cycle_name,config)
    print("--------------------------calling update_tr_multiple----------------------------------------------------")
    update_tr_multiple(have_tr, test_cycle_name, config, status_value, reason_value, owner)
    print("---------------------------calling create_tr_multiple---------------------------------------------------")
    create_tr_multiple(not_have_tr, status_value, reason_value, link_type, owner, test_cycle_name, config, release)