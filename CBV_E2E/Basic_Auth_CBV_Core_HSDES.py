                                    ...
                                    if "central_firmware.feature.val_assessment_reason" in resp.json()["data"][0]:
                                        if resp.json()["data"][0]["central_firmware.feature.val_assessment_reason"] == "testable_requirement":
                                            ...
                                            if feature_links_response.status_code == 200:
                                                ...
                                                #print(tcd_data)
                                                for m in tcd_data:
                                                    s= "https://hsdes.intel.com/appstore/article/#/"+m['id']
                                                    #s = '\"https://hsdes.intel.com/appstore/article/#/{0}\", \"{1}\")'.format(m['id'], m['id'])
                                                    final_tcd_data.append([m['id'], s,m['title']])
                                                    #final_data[str(list_hsd[i])].append([s, m['title']])
                                            else:
                                                print("ISE's parent (feature) links response not valid.")
                                        else:
...
#                    print("Inside Bug - {0}".format(result['data'][0]['id']))
                    ...
                    if ('client_platf.bug.test_case_id' in result['data'][0]) or ('central_firmware.bug.test_case_id' in result['data'][0]):
                        ...
                        if(bug_test_case_id_field_data is not None):
                                ...
                                if bug_tc_id != "":
                                    ...
                                    if bug_tc_id_response.status_code == 200:
                                        ...
                                        if bug_tc_id_response.json()['data'][0]['subject'] == 'test_case_definition':
                                            ...
                                            #s = ['\"https://hsdes.intel.com/appstore/article/#/{0}\", \"{1}\")'.format(bug_tc_id_response.json()['data'][0]['id'], bug_tc_id_response.json()['data'][0]['id'])
                                            s = "https://hsdes.intel.com/appstore/article/#/"+ bug_tc_id_response.json()['data'][0]['id']
                                            #final_tcd_data.append([s, bug_tc_id_response.json()['data'][0]['title']])
                                            final_tcd_data.append([bug_tc_id_response.json()['data'][0]['id'],s, bug_tc_id_response.json()['data'][0]['title']])
                                        ...
                                        if 'parent_id' in parent_tc_id_response.json()['data'][0]:
                                            ...
                                            if parent_tcd_id_response.status_code == 200:
                                                if parent_tcd_id_response.json()['data'][0]['subject'] == 'test_case_definition':
                                                    ...
                                                    #s = '\"https://hsdes.intel.com/appstore/article/#/{0}\", \"{1}\")'.format(parent_tcd_id_response.json()['data'][0]['id'], parent_tcd_id_response.json()['data'][0]['id'])
                                                    s = "https://hsdes.intel.com/appstore/article/#/" + parent_tcd_id_response.json()['data'][0]['id']
                                                    #s = '\"{0}\",\"https://hsdes.intel.com/appstore/article/#/{1}\")'.format(parent_tcd_id_response.json()['data'][0]['title'], parent_tcd_id_response.json()['data'][0]['id'])
                                                    #final_tcd_data.append([s, parent_tcd_id_response.json()['data'][0]['title']])
                                                    final_tcd_data.append([parent_tcd_id_response.json()['data'][0]['id'],s, parent_tcd_id_response.json()['data'][0]['title']])
                                                    #final_tcd_data.append([parent_tcd_id_response.json()['data'][0]['id'],s, parent_tcd_id_response.json()['data'][0]['title']])
                                                    #final_data[str(list_hsd[i])].append([s, parent_tcd_id_response.json()['data'][0]['title']])
                                                    #final_data[str(list_hsd[i])].append([s, parent_tcd_id_response.json()['data'][0]['id']])
                                            else:
    ...
    for element in final_tcd_data_unique:
        final_tcd_data_dict[element[0]] = [element[2],element[1]]
    ...
    df = pd.DataFrame(final_tcd_data_dict, columns = ["Test_Case_Definition_ID", "Test_Case_Definition_Title",])
    df.to_excel("/opt/ybobase/output.xlsx", index=False) 
...
def main():
    ...
    final_tcd_data_dict = get_tcd("Config.JSON", "test.txt", "a44ba54cff2", "8956d4fbf2e")
    print("In main: ", final_tcd_data_dict)
    #pprint.pprint(final_tcd_data_dict)
